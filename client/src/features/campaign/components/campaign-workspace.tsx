...
import {
  useCampaignLineage,
  useCampaignPackage,
} from "../hooks/use-campaign-records";
...
export function CampaignWorkspace({ onOpenVerifier }: CampaignWorkspaceProps) {
  ...
  const [beats, setBeats] = useState<BeatProgress[]>(() =>
    createInitialBeats(),
  );
  ...
  const campaignPackageQuery = useCampaignPackage(
    jobId,
    campaignQuery.data?.status,
  );
  const campaignLineageQuery = useCampaignLineage(
    jobId,
    campaignQuery.data?.status,
  );
  ...
  useEffect(() => {
    ...
    if (campaign.beat_plan) {
      setBeats(
        campaign.beat_plan.beats.map((beat, index) => {
          const completedBeat = campaign.beats.find(
            (item) => item.index === beat.index,
          );
          return {
            index: beat.index,
            label: `Slide ${index + 1}`,
    ...
    if (campaign.status === "failed")
      handleStreamFailure(campaign.error ?? "Generation failed");
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Could not verify this run",
      );
    }
  };
      ...
      <section className="border-b border-[#e5e2dc] bg-[#faf9f6] lg:max-h-[calc(100vh-6.75rem)] lg:overflow-y-auto lg:border-r lg:border-b-0">
        <div className="p-5 sm:p-7">
          <div className="mb-7 flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#77737a]">
                Campaign brief
              </p>
              <h1 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-[#20202b]">
                Create a reel
              </h1>
              <p className="mt-2 max-w-md text-sm leading-6 text-[#6e6b75]">
                Set the message and visual direction. Your working preview stays
                alongside it.
              </p>
            </div>
      ...
      <section className="min-w-0 bg-white p-5 sm:p-7">
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-xl font-semibold tracking-[-0.03em] text-[#20202b]">
              Preview & activity
            </h2>
          </div>
          <Badge
            variant="outline"
            className="h-7 border-[#dedbd4] bg-[#fafaf8] px-2.5 text-[#66626c]"
          >
            {workspaceStatus}
          </Badge>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(19rem,0.85fr)]">
          <div className="min-w-0">
            {campaignQuery.data && stage === "complete" ? (
              <CampaignResult
                ...
                campaignLineage={campaignLineageQuery.data}
                campaignLineageError={campaignLineageQuery.error}
                campaignPackage={campaignPackageQuery.data}
                campaignPackageError={campaignPackageQuery.error}
                isCampaignLineageLoading={campaignLineageQuery.isLoading}
                isCampaignPackageLoading={campaignPackageQuery.isLoading}
                verification={verification.data}
                isVerifying={verification.isPending}
                onVerify={() => void handleVerification()}
              ...
              <div className="flex min-h-[25rem] flex-col justify-between overflow-hidden rounded-xl border border-[#e2dfd8] bg-[#f8f7f4] p-5 sm:p-7">
                <div className="flex items-center justify-between gap-3">
                  <Badge
                    variant="outline"
                    className="border-[#dedbd4] bg-white text-[#625f68]"
                  >
                    9:16 {mode === "pov" ? "POV montage" : "slideshow"}
                  </Badge>
                ...
                <div className="max-w-sm">
                  ...
                  <h3 className="text-xl font-semibold tracking-[-0.03em] text-[#2a2934]">
                    Your reel will appear here.
                  </h3>
                  <p className="mt-2 text-sm leading-6 text-[#706d76]">
                    Write a brief, add optional product images, then generate a
                    storyboard to begin the review cycle.
                  </p>
                </div>
                <div className="border-t border-[#e4e1db] pt-4 text-xs text-[#77737c]">
                  The final export and its provenance record stay together.
                </div>
              </div>
            )}
          </div>
          ...
          <aside className="min-w-0 rounded-xl border border-[#e2dfd8] bg-[#fcfbf9] p-4 sm:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-[#33313a]">
                <History className="size-4 text-[#74707a]" /> Run activity
              </div>
