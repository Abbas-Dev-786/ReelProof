...
export function LineageTimeline({ lineage, compact = false }: LineageTimelineProps) {
  if (lineage.length === 0) {
    ...
    <ol className="space-y-0">
      {lineage.map((entry, index) => (
        <li className="relative flex gap-3 pb-5 last:pb-0" key={entry.run_id}>
          {index < lineage.length - 1 && <span aria-hidden="true" className="absolute left-[13px] top-7 h-[calc(100%-1.2rem)] w-px bg-[#ded9e8]" />}
          <span className="relative z-10 grid size-7 shrink-0 place-items-center rounded-full bg-[#eeeaff] text-[#6954c6]">
            {index === lineage.length - 1 ? <ShieldCheck className="size-3.5" /> : <GitBranch className="size-3.5" />}
          </span>
          <div className="min-w-0 pt-0.5">
            <p className="text-sm font-medium text-[#383541]">{entry.parent_run_id ? "Refined generation" : "Initial generation"}{index === lineage.length - 1 && <span className="ml-2 inline-flex items-center gap-1 text-xs font-medium text-[#278052]"><CheckCircle2 className="size-3" />Verified</span>}</p>
